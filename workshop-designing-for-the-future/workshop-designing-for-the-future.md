...
4. **Share and diagnose** (~15 min) — as a group, look at what didn't work and why. Brainstorm ways to improve both the prompts and the group's expectations of the tools.
5. **Reflect** (~15 min) — what would it take to be more specific? What did historical futurists do to articulate their futures clearly that a prompt alone doesn't capture? What stories are these tools actually drawing from when they render "the future"? This is usually where the broad-scene-vs-specific-object problem surfaces — worth naming directly if it hasn't come up yet, and pointing at Julian Bleecker and Near Future Laboratory's design fiction work as the model: a single, specific object implies a whole world far better than a wide shot of one does.
...
Undergraduate studio, any level — ran originally with graphic design juniors. Graduate seminar. Works for a single class section (12–25 students); needs laptops and a shared collaborative document (Figma or equivalent) for the "alone together" exercises.

## What they leave with

Multiple AI-generated images testing their own vision of a preferable future, with prompts and tools logged for each. A working knowledge of at least one futures-studies framework (Stuart Candy's four generic futures). A tested, specific sense of what current text-to-image tools handle well and where they fall apart.

## What you need

- A single afternoon at minimum (the original ran 4:15–7:00pm, tight but workable) — shorter than the other workshops on this menu. Can expand to two or three days for real reflection time between rounds and actual iterative improvement on the images, rather than one pass straight through all eight steps.
- Laptops for all students
- A shared collaborative document tool (Figma was used originally) with templates for logging images/prompts
- Access to at least one text-to-image tool — which one(s) is still an open, evolving question given how fast this space moves
- A projector for group share-outs
...
- Western Michigan University, October 9, 2023 — "Design the Future Today," run with graphic design juniors as a single afternoon session following a lecture. This is real, tested material with a full run script. Timing ran tight but worked out.
...
- "Which AI tool(s)" is a genuinely moving target — The general category of tool is the "text-to-image diffusion" stuff, I don't particularly care about a specific tool.
